...
def _collapse_repeated_text(text: str) -> str:
    """Drop repeated sentences from a single LLM reply.

    The conversation model sometimes emits its whole answer twice in one
    generation ("<line>\\n\\n<line>"), which TTS then reads aloud twice. We keep
    only the FIRST occurrence of each sentence (by normalized form), collapsing
    any such repeat while leaving a normal, non-repeating reply untouched.
    """
    seen: set[str] = set()
    out: list[str] = []
    end = 0
    ...
    if tail and not (_dedup_key(tail) and _dedup_key(tail) in seen):
        out.append(tail)
    return "".join(out)
