...
# Splits text into sentences, keeping each sentence's trailing terminator(s)
# (Devanagari danda, ?, !, newlines) so re-joining is lossless.
_SENTENCE_SPLIT = re.compile(r"[^।!?\n]*[।!?\n]+")


def _tokens_similar(a: list[str], b: list[str], threshold: float = 0.7) -> bool:
    """True if two sentences are near-duplicates by token overlap.

    Catches a REWORDED repeat (e.g. "... कब तक कर पाएंगे?" then "कृपया बताएं ...
    कब तक कर पाएंगे?") that exact-match dedup misses. Directional (one side
    almost contained in the other) so an echo with a few extra words still
    counts. Only applied to sentences of >= 5 tokens, so distinct short lines
    ("धन्यवाद Pradeep जी") are never wrongly merged.
    """
    ...
    if not inter:
        return False
    return max(inter / len(aset), inter / len(bset)) >= threshold


def _collapse_repeated_text(text: str) -> str:
    """Drop repeated (and near-repeated) sentences from a single LLM reply.

    The conversation model sometimes emits its whole answer twice in one
    generation ("<line>\\n\\n<line>"), or re-asks the same question reworded,
    which TTS then reads aloud twice. We keep only the FIRST occurrence of each
    sentence — by normalized form OR by high token overlap — collapsing any such
    repeat while leaving a normal, non-repeating reply untouched.
    """
    seen: set[str] = set()
    ...
    def _is_repeat(sentence: str) -> bool:
        ...
        if any(_tokens_similar(toks, k) for k in kept_tokens):
            return True
        seen.add(key)
    ...
    for m in _SENTENCE_SPLIT.finditer(text):
        ...
        sentence = m.group(0)
        if _is_repeat(sentence):
            continue
        out.append(sentence)
    tail = text[end:]  # trailing text with no terminator
    if tail and not _is_repeat(tail):
        out.append(tail)
    return "".join(out)


# Sentences where the model narrates call mechanics / its own internal state
# instead of speaking to the customer. These must never be read aloud (e.g.
# "ग्राहक से कुछ response नहीं मिला है, मैं प्रतीक्षा करूँगी").
...
def _content_tokens(text: str) -> list[str]:
    ...
    `\\w` matches Devanagari letters under Python's default Unicode rules.
    """
...
def _last_assistant_text(turn_ctx) -> str:
    """The agent's most recent spoken line from the chat context (or '')."""
    for item in reversed(getattr(turn_ctx, "items", []) or []):
...
class Assistant(Agent):
    def __init__(self) -> None:
        ...
        super().__init__(
            ...
            instructions=build_agent_instructions(
                customer_name=LOADED_CUSTOMER_NAME,
                customer_context=CUSTOMER_CONTEXT,
                relative_dates=relative_dates,
                today_str=now.strftime("%d-%m-%Y"),
                today_weekday=_HINDI_WEEKDAYS[now.weekday()],
                tomorrow_spoken=_spoken_hindi_date(now + timedelta(days=1)),
            ),
        )
    ...
    async def llm_node(self, chat_ctx, tools, model_settings):
        """Collapse a self-repeated reply before it reaches TTS/transcript.

        The conversation model sometimes emits its whole answer twice in one
        generation ("<line>\\n\\n<line>"), which TTS would otherwise read aloud
        twice. We buffer the streamed text, drop any repeated sentence, and emit
        the cleaned text once. Tool-call and non-text chunks pass straight
        through untouched.
        """
        base = Agent.default.llm_node(self, chat_ctx, tools, model_settings)
    ...
    async def on_user_turn_completed(self, turn_ctx, new_message) -> None:
        ...
        Backstops behind stt_node for the turns that make the agent re-ask /
        loop:
          * an empty or filler-only turn (nothing was actually said),
          * a turn that echoes the agent's OWN last line (its TTS bleeding back
            as 'user' speech — the cause of the back-to-back re-asked question),
          * an exact repeat of the previous user turn (STT double-final).
        """
        text = (new_message.text_content or "").strip()

        if not text or _is_filler_only(text):
            logger.info(">>> EMPTY/FILLER TURN dropped: %r", text)
            raise StopResponse()

        last_agent = _last_assistant_text(turn_ctx)
        if last_agent and _is_echo_of(text, last_agent):
            logger.info(">>> ECHO TURN dropped: %r (echoes agent's last line)", text)
            raise StopResponse()
        ...
        logger.info(">>> USER turn accepted: %r", text)
